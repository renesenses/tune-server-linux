...
for /f "tokens=*" %%t in ('powershell -NoProfile -Command "try { (Invoke-RestMethod -Uri 'https://api.github.com/repos/renesenses/tune-server-linux/releases/latest' -UseBasicParsing).tag_name.TrimStart('v') } catch { Write-Output '0.8.88' }"') do set "VERSION=%%t"
if "%VERSION%"=="" set "VERSION=0.8.88"
