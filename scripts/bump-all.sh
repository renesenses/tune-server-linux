...
# 6. Homebrew formula — DISABLED: the Rust formula in homebrew-tap is now
# maintained separately (with per-arch SHA256). The old Python formula in
# tune-server-linux/homebrew/ is legacy and must NOT overwrite the tap.
# SHA256 for the Rust tarballs are computed after CI and pushed manually.
echo "  - homebrew-tap: update manually after CI (Rust formula)"
