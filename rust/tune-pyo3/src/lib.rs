...
#[pyfunction]
fn read_metadata(py: Python<'_>, path: &str) -> PyResult<Option<Py<PyAny>>> {
...
#[pyfunction]
fn build_wav_header(py: Python<'_>, channels: u16, sample_rate: u32, bit_depth: u16) -> Py<PyAny> {
    let header = tune_core::audio::wav::build_wav_header(channels, sample_rate, bit_depth);
    PyBytes::new(py, &header).unbind().into_any()
}
