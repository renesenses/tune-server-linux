...
impl RustDatabase {
    ...
    fn artist_search<'py>(&self, py: Python<'py>, query: &str, limit: i64) -> PyResult<Vec<Py<PyAny>>> {
        let repo = ArtistRepo::new(self.db.clone());
        let artists = repo.search(query, limit)
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e))?;
        artists.iter().map(|a| Ok(artist_to_dict(py, a)?.unbind().into_any())).collect()
    }
    ...
    fn track_search<'py>(&self, py: Python<'py>, query: &str, limit: i64) -> PyResult<Vec<Py<PyAny>>> {
        let repo = TrackRepo::new(self.db.clone());
        let tracks = repo.search(query, limit)
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e))?;
        tracks.iter().map(|t| Ok(track_to_dict(py, t)?.unbind().into_any())).collect()
    }
